...
*/
{
  //thread_local std::mt19937 generator(std::random_device{}()); //Commented out 6/21 - J.S.
  thread_local std::mt19937 generator(137+omp_get_thread_num()); //Added 6/21 - J.S. //Modified 2/27/24 - N.V.

  //J.S.: Note the changes above were done to reduce/eliminate the randomness when generating templates.
  //      Randomness could lead to alternating SNR signs, so I hope that seeding the mt199937 would reduce this.

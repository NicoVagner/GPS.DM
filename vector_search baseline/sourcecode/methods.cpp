...
*/
{
  ...
  ,best_h=0,best_n0,best_n1,best_n2;
  double best_O=-1;
  ...
  best_params.push_back(best_n0);
  best_params.push_back(best_n1);
  best_params.push_back(best_n2);
  ...
  paramFile<<"t0\todds\th\tv\ttheta\tphi\td\tR\ta\tn\n";
  paramFile.open(eventParams, std::ios_base::app);
    for(int i_prm=0; i_prm<12;i_prm++){//fix!need->10 for mono, etc.
...
/*
2/28/2022 (Jiten) 
    - This function is very similar to likeParameterEstimations (which is much more readable)
    - 
*/
  ...
  int exact, double * Einv, int invest)
{
  ...
  prepareData(hJW,j0,jmin,dD,rsat,rref,data);
    ...
    double chi_dd=0.;
     //  chi_dd = calcChi(dD,dD,Hijl,data.sdev,JW,data.num_clocks,icov);
       //Include the cross-clock-correlation term:
    //   if(use_ccc) chi_dd += calcChiW(dD,dD,b0,data.sdev,JW,data.num_clocks);
        
       //chi_dd = calcChi(dD, dD, Hijl, data.sdev, hJW, data.num_clocks, icov);
       //   chi_dd += calcChiW(dD,dD,sb0,data.sdev,hJW,data.num_clocks);
  ...
  // Monte Carlo integration setup begins here:
  double bestParams[12]={0}; //safely initializing the bestParams vector to be 0 in all positions
    double ng[3]; //predicted DM impact vector
    double psi; //holds "psi" angle, which is between signal template's "n[]" and predicted DM direction "ng[]"
    double ng_dot_n; //used in calc. for psi angle
    double mag_ng_n; //used in calc. for psi angle
    
  // Monte Carlo integration beings below:  
  for (int ix=0; ix<NMC; ix++){
    double t0,v,d,R,a;  //  Randomly choose integration params:
                        //  time, vel, width, impact param, impact angle
                        //  Please note: width, impact param, and impact angle are not necessary for int snrParameterEstimation
                        //  they are generated here only so that we can reuse randomParameters() and generateSignal() w/o making an overloaded function
    double n[3]; //incident direction vector of DM object, (x,y,z) in ECI

    randomParameters(v_prior,psi_prior,model,j0,t0,v,n,d,R,a); // This function randomly chooses integration parameters
                                                               // which are then "fed" into the generateSignal() function

    //generate signal:
    double s[MAXCLOCKS][MAXJW];//holds signal
    generateSignal(model,s,rsat,rref,data.num_clocks,jmin,JW,
                   data.vdiff,data.keff,t0,v,n,d,R,a);         // This function creates the signal template based on randomly chosed integration params.
/*
    // tday 06/28/21 to check if we get same parameters as put in
      double teDM[3];
      double tvDM;
      
      std::string path_to_sig2 = "./results/injSignalParams13101.txt";
      std::ifstream inFilet2;
      inFilet2.open (path_to_sig2.c_str());
          if(inFilet2.is_open()){
              inFilet2 >> teDM[0] >> teDM[1] >> teDM[2] >> tvDM;
          }
      
      inFilet2.close();
      
      // these are the same exact parameters as injected
      n[0] = teDM[0];
      n[1] = teDM[1];
      n[2] = teDM[2];
      v = tvDM;
  */
/*    
      std::string path_to_sig = "./results/injSignalTemplate14101.txt";
          
          double ss[data.num_clocks][MAXJW];
    ...
    // end of 06/28/21 check for same template parameters
*/
    //Subtract the weighted mean
    // s -> s - s_bar
    if(sub_w_mean)
    ...
    double tempsnrsd=1/sqrt(sHs);   // added 12/8/20 to store the h-hat sd.
      ...
      double h;
      h = dHs/sHs;
      chi_val[ix]  = chi_dd -2*h*dHs + (h*h)*sHs;


    //Perform the "maximisation"
    //update best-fit parameters
    if(fabs(tempSnr) > fabs(bestParams[1])){  //  This line compares the current SNR to the previous SNR
	    bestParams[0]=t0;
      bestParams[1]=tempSnr;
	    bestParams[2]=dHs/sHs;
	    bestParams[3]=v/30.;  //  current best speed
	    bestParams[4]=acos(n[2])/M_PI;  //  current best theta/pi
                                      //  Recall: z = cos(theta) for a unit sphere, so acos(z) = theta
                                      //  Divided by pi for the graphs used in Bayesian methods paper, not necessary though
                                      //  as division by pi is not part of the calc. for the angle "theta"
      double tmp_phi=atan2(n[1],n[0]);
      if(tmp_phi<0) tmp_phi += 2*M_PI;
      bestParams[5]=tmp_phi/M_PI; //  current best phi/pi (the above two lines is an elementary trigonometric check when using atan())
	    bestParams[6]=n[0]; //  x
	    bestParams[7]=n[1]; //  y
	    bestParams[8]=n[2]; //  z
        ...
        //  these values are in the ECI J2000 frame of reference
        ng[0] = 0.46;
        ng[1] = -0.49;
        ng[2] = 0.74;
        ...
        psi = acos(ng_dot_n/mag_ng_n); // this is the angle psi
        
        bestParams[9] = psi;  //  current best psi
        // end of directionality angle updated on 11/20/20
        bestParams[10] = tempsnrsd;
        bestParams[11] = chi_val[ix];
	  //std::cout << "ix: " << ix << "\n";
  ...
  //std::ofstream paramFile;
    if(itn_or_day <= 12000){ //tests if in testMethods program or search program
      ...
      //The above line is commented out so that all parameter estimations are appended to one giant file, 1/7/2021 - Jiten
      std::string eventParams = "./results/parameterEstimations.out"; //This line should be commented out to return to single-day P.E. - Jiten
      std::ofstream paramFile;
      paramFile.open(eventParams, std::ios_base::app);
      if(invest){
        ...
        for(int i_prm=0; i_prm<12;i_prm++){//fix!need->10 for mono, etc.
          paramFile<<bestParams[i_prm]<<"\t";
          if(i_prm==11) paramFile<<std::endl;
        }
        paramFile<<"\n";
...
{
/*
 this function is to build a template bank that is nearly orthogonal
 tday June 2021
 */
    std::cout << "Hello in template bank" << "\n";
    std::cout << "j0: "<<j0 << "\n";
    std::string nj0 = std::to_string(j0);
     std::cout << "nj0: "<<nj0 << "\n";
    int size = JW*data.num_clocks;
    std::vector< std::vector<double> > oEij(size,std::vector<double>(size));

    std::string path_to_sig = "./results/signal_tempbank_centralref"+nj0+".txt";
    std::ofstream inFilet;
    inFilet.open (path_to_sig.c_str());
    
    std::string path_to_sig2 = "./results/signal_corrtempbank_centralref.txt";
    std::ofstream inFilet2;
    inFilet2.open (path_to_sig2.c_str());

    std::string path_to_sig3 = "./results/signal_tempbank_params"+nj0+".txt";
    std::ofstream inFilet3;
    inFilet3.open (path_to_sig3.c_str());


/*    

    std::cout << "Hello 2 in template bank" << "\n";
    std::cout << "NMC: " << NMC << "\n";
    std::cout << "Hello 3 in template bank" << "\n";
    std::cout << "num clocks: " << data.num_clocks << "\n";
    std::cout << "MAXJW: " << MAXJW << "\n";
    std::cout << "j0: " << j0 << "\n";
*/           
   // double ss[data.num_clocks*NMC][MAXJW];
    std::vector< std::vector<double> > ss(data.num_clocks*NMC,std::vector<double>(MAXJW));
    //long double ss[data.num_clocks*NMC][MAXJW];
           

/*

   // this part is to inject a w.n. data streams into the bias instead of colored noise network
   std::vector< std::vector<double> > dd(data.num_clocks,std::vector<double>(2880));

   std::string path_to_sig3 = "./whitenoise_refpoint1sat_ratio_biasdif_zerod2.txt";
   std::ifstream inFilet3;
    inFilet3.open (path_to_sig3.c_str());
        if(inFilet3.is_open()){
            for(int i = 0; i<data.num_clocks; i++){
  ...
  //  end of w.n. data stream injection

*/
   
  //  std::cout << "Hello 4 in template bank" << "\n";
    double corthresh;
    
    
  //  corthresh = 0.5/(NMC);
    corthresh = 0.33;
  //  corthresh = 2.0/sqrt(data.num_clocks);
    
  //  std::cout << "this is corthresh; " << corthresh << "\n";
 
    // new variable for the delta_corr
    double deltacorr;
    deltacorr = 0.05;    
    ...
    double dD[data.num_clocks][MAXJW];//careful, trailing junk!
     double rsat[data.num_clocks][3];
     double rref[3];
     int jmin;
     prepareData(thJW,j0,jmin,dD,rsat,rref,data);
  ...
  //    double bestParams[11]={0};
    ...
    //    std::cout << "Hello Tyler this is NMC for template bank: " << NMC << "\n";
    
    int kl;
    kl = 0;  

    std::vector< double > corrij(NMC*NMC);
    std::vector< double > totcorrij(NMC*NMC);
    std::vector< double > denomcorrijl(NMC*NMC);  
    std::vector< double > denom2corrijl(NMC*NMC);


    std::vector< double > tempvel(NMC);
//    std::vector<std::vector<std::vector<std::vector< double >>>> tempn(NMC,std::vector<double> (1), std::vector<double> (1),std::vector<double>(1));  
    std::vector<std::vector< double >> tempn(NMC,std::vector<double> (3)); 
...
//    printf("Parallelised (OMP) using %i cores\n\n",ilimit_cores);
    #pragma omp parallel for default(shared) shared(kl,ss,j0,corrij,totcorrij,denomcorrijl,denom2corrijl) ordered
      for (int ix=0; ix<NMC; ix++){
        ...
        //generate signal:
          double s[data.num_clocks][MAXJW];//holds signal
        generateSignal(model,s,rsat,rref,data.num_clocks,jmin,JW,
                       data.vdiff,data.keff,t0,v,n,d,R,a);
       
          // for first template we store it without checking
          if(ix == 0){
//	      std::cout << "template #: " << ix << "\n";
              for(int a=0;a<data.num_clocks;a++){
                  for(int b=0;b<JW;b++){
//                      ss[a][b] = s[a][b];
                      ss[a + ix*data.num_clocks][b] = s[a][b];
      //                std::cout << ss[a][b] << " " ;
                  }
      //            std::cout << "\n";
              }
          }
          // end of first template storage to ss.
          
          // now to check the correlation for each new template s
      //    double corrij[NMC*NMC];
      //    double totcorrij[NMC*NMC];
 //         std::vector< double > corrij(NMC*NMC);
 //         std::vector< double > totcorrij(NMC*NMC);
          double corrtmp;
          ...
          double denomtmp;
   //       double denomcorrijl[NMC*NMC];
//          std::vector< double > denomcorrijl(NMC*NMC);
          
          double denom2tmp;
   //       double denom2corrijl[NMC*NMC];
 //         std::vector< double > denom2corrijl(NMC*NMC);
         
          
          // check correlation between templates
          if(ix > 0){
              int tcount;
              tcount =0;
              
//             kl=0;
              // need to include the network covariance matrix with the tempaltes
            #pragma omp ordered
            {
              for(int ixl =0; ixl<ix; ixl++){
                  
                  
  	//	  std::cout << "ix: " << ix << "\n";         
        //          std::cout <<"ixl: " << ixl << "\n";
              // this part calculates the numerator of the SNR cov matrix
              
	//	std::cout << "template #: " << ixl << "\n";
              for(int a=0;a<data.num_clocks;a++){
                  for(int b=0;b<JW;b++){
//                      ss[a][b] = s[a][b];
               //       ss[a + ix*data.num_clocks][b] = s[a][b];
//                      std::cout << ss[a+ ixl*data.num_clocks][b] << " " ;
                  }
//                  std::cout << "\n";
              }
                  ...
                  int ml=0;
                    ...
                    corrtmp = 0;
                    for(int i=0;i<data.num_clocks;i++){
                        for(int k =0; k<data.num_clocks;k++){
                            
                           // ml = 0;
                            for(int j=0;j<JW;j++){
                                for(int l=0;l<JW;l++){
                                 //corrtmp += s[i][l]*(*(Einv + (i*JW + j)*size + (k*JW + l)))*ss[i + (ixl*data.num_clocks)][l];
                                 //corrtmp += s[i][j]*oEij[i*data.num_clocks + j][k*data.num_clocks + l]*ss[k + (ixl*data.num_clocks)][l];
				 corrtmp += s[i][j]*oEij[i*JW + j][k*JW + l]*ss[k + (ixl*data.num_clocks)][l];
                                    ml++;
                                }
                            }
                           // corrij[kl] = corrtmp/(ml);
                           // kl++;
                        }
      ...
      //            std::cout << "this is corrij[kl]: " << corrij[kl] << "\n";
                    
                    // end of part that calculates numerator
                    
                    // calculate the denominator of template i
                    ...
                    for(int i=0;i<data.num_clocks;i++){
                        for(int k =0; k<data.num_clocks;k++){
                          //  denomtmp = 0;
                            ...
                            for(int j=0;j<JW;j++){
                                for(int l=0;l<JW;l++){
                                 //denomtmp += s[i][l]*(*(Einv + (i*JW + j)*size + (i*JW + l)))*s[i][l];
                                    denomtmp += s[i][j]*oEij[i*JW + j][k*JW + l]*s[k][l];
				    //denomtmp += s[i][j]*oEij[i*data.num_clocks + j][k*data.num_clocks + l]*s[k][l];
                                    ml++;
                                }
                            }
                          //  denomcorrijl[kl] = denomtmp/ml;
                           // kl++;
                        }
                        
                    }
                        denomcorrijl[kl] = denomtmp;
                  
      //            std::cout << "this is denomcorrijl[kl]: " << denomcorrijl[kl]<< "\n";
                    
                    // end of denominator for template i
                    
                    // calculate the denominator of template j
                    ...
                    for(int i=0;i<data.num_clocks;i++){
                        for(int k =0; k<data.num_clocks;k++){
                            ...
                            for(int j=0;j<JW;j++){
                                for(int l=0;l<JW;l++){
                                 //denom2tmp += ss[i + (ixl*data.num_clocks)][l]*(*(Einv + (i*JW + j)*size + (i*JW + l)))*ss[i + (ixl*data.num_clocks)][l];
				  denom2tmp += ss[i+ (ixl*data.num_clocks)][j]*oEij[i*JW + j][k*JW + l]*ss[k + (ixl*data.num_clocks)][l];
                                  //denom2tmp += ss[i+ (ixl*data.num_clocks)][j]*oEij[i*data.num_clocks + j][k*data.num_clocks + l]*ss[k + (ixl*data.num_clocks)][l];
                                    ml++;
                                }
                            }
                           // denom2corrijl[kl] = denom2tmp/ml;
                          //  kl++;
                        }
                        
                    }
                        denom2corrijl[kl] = denom2tmp;
                  
   //               std::cout << "this is denom2corrijl[kl]: " << denom2corrijl[kl]<< "\n";
                    
                    // end of denominator for template j
                    
                    // now divide the numerator by the sqrt of denom i and j
                  //  for(int i =0;i<kl;i++){
                        totcorrij[kl] = corrij[kl]/sqrt(denomcorrijl[kl]*denom2corrijl[kl]);
                  
     //             std::cout << "this is totcorrij[kl]: " << totcorrij[kl]<< "\n";
                   // }
               //   if(fabs(totcorrij[kl])<corthresh){
                  //if(fabs(totcorrij[kl])<=corthresh+deltacorr && fabs(totcorrij[kl])>=corthresh-deltacorr){
		  if(fabs(totcorrij[kl])< 0.9 && fabs(totcorrij[kl])>0.1){
                      for(int i=0;i<data.num_clocks;i++){
                          for(int j=0; j<JW;j++){
                              ss[i + (ix*data.num_clocks)][j] = s[i][j];
         ...
         //            std::cout << "kl: " << kl << "\n";
			// continue;
		      ...
		      ixl = -1;
                 ...
                 // kl++;
                  count++;
                    // end of final SNR cov matrix element ij calculation
              
              }
              // end of ixl loop
            } // end of pragma omp ordered
              
              // set the template
          /*    
              for(int i =0;i<kl-1;i++){
                  if(fabs(totcorrij[i])<corthresh){
                      for(int a=0;a<data.num_clocks;a++){
                          for(int b=0;b<JW;b++){
                              ss[a+(ix*data.num_clocks)][b] = s[a][b];
                          }
              ...
              for(int i=0;i<count;i++){
                     inFilet2 << corrij[i] << " "<< denomcorrijl[i] << " "<< denom2corrijl[i] << " " << totcorrij[i] << "\n";
                 }
            */  
              
          }
          // end of checking for a correlation below 49/(M^2)
...
*/     
    ...
    std::cout << "Here at end of template build" << "\n";
    
    
    // now fill the template bank with all of the nearly orthogonal templates
    for(int i=0;i<(data.num_clocks*NMC);i++){
        for(int j=0;j<JW;j++){
            inFilet << ss[i][j] << "    ";
        }
        inFilet << "\n";
    }
    // end of filling new nearly orthog template bank
 
    // now fill in the template parameters
    for(int i=0;i<NMC;i++){
	inFilet3 << tempvel[i] <<" "<< tempn[i][0] <<" "<< tempn[i][1] <<" "<< tempn[i][2] << "\n";
    }
...
//  t0 = MFS_randDouble(double(j0-30),double(j0+30));   // added 06/17/21 by tday as a test for publication
  //t0 = j0;   // changed on 06/11/21 tday

  //Choose the incident |v| using the numeric priors
  {
    //double uv=MFS_randDouble(0.001,1.);
    double uv=MFS_randDouble(0.0089,1.); 		// this is for speeds ~28km/s such that each clock affected in window
    v = v_prior.inverseCdf(uv)*30.; //double check x30! tau_0

	// adding this section as a hard check for minimum velocities being under the assoiciated Jw speed
    if(v < 820.0){
 	uv = MFS_randDouble(0.01+0.001,1.);
	v = v_prior.inverseCdf(uv)*30;
	v = 820.0;	// could maybe do just hard force to minimum velocity (27.32 km/s for Jw = 61)
    } 
	// end of new section as rough hard check plus re-making the velocity

  }

  // Angles (direction) eDM = n
  //Note: we choose the angles in the frame that has z along n_g (direction of motion through galaxy)
  ...
  // the ECI frame.
  {
    ...
    //Rotate back into ECI (theta/phi) frame (not sure when this was last checked - Jiten):
    double cp   = cos(psi); //  getting z from spherical coordinates in (psi,omega) frame
    double cosp = cos(omega)*sin(psi);  //  getting x from spherical coordinates in (psi,omega) frame
    double spso = sin(psi)*sin(omega);  //  getting y from spherical coordinates in (psi,omega) frame
    n[0] = -0.46332*cp + 0.179476*cosp + 0.867827*spso; //  x in ECI (theta,phi) frame
    n[1] =  0.49003*cp + 0.867827*cosp + 0.082144*spso; //  y in ECI (theta,phi) frame
    n[2] = -0.73838*cp + 0.463320*cosp - 0.490030*spso; //  z in ECI (theta,phi) frame
  }
...
// added 03/22/20 for pos. def. cov matrix test
  std::vector< std::vector<double> > oEij(size,std::vector<double>(size));
    ...
    // this is for adding a w.n. covariances to the cov matrix from a file
    
    std::string path_to_sig4 = "./whitenoise_refpoint1sat_ratio_biasdif_zerod2_variancesoutput.txt";
        
        double tvr[1][data.num_clocks];
        double tvari[data.num_clocks];        

    std::ifstream inFilet4;
    inFilet4.open (path_to_sig4.c_str());
        if(inFilet4.is_open()){
            for(int i = 0; i<data.num_clocks; i++){
    ...
    // end of w.n. covariance addiyion on 06/18/21 tday
      ...
      for(int a=0; a<data.num_clocks; a++){
        for(int b=a+1; b<data.num_clocks; b++){
            for(int j=0; j<jw; j++){
                ...
                *( Einv + (a*jw+j)*size + (b*jw+j) ) = tref_sig;
                *( Einv + (b*jw+j)*size + (a*jw+j) ) = tref_sig;
                //*(Einv+j*size+l)=arrayE[j][l]; conversion
            }
        }
      }
...
Eout.close();
  ...
  //======================================================================== 
    ...
    // added on 01/04/21 tday
        if(info!=0){
          std::cout<<"Here: FAILURE 477 in genHijl: Eij matrix could not be inverted!\n";
          if(info<0){
...
//  std::cout << "this is keff[num_clocks]: " << keff[num_clocks] << "\n";
 ...
 //   std::cout << "jmin: " << jmin << "\n";
 //   std::cout << "xi_ref: " << xi_ref << "\n";
//  double tr=t0-jmin-(xi_ref/v);  //time object centre hits the reference clock
  double tr=t0 - jmin;  //time object centre hits the reference clock (06/11/21 tday)
...
*/
{
    ...
    std::string bchiPath = "./results2/signal_1.out";
...
//	std::cout << "ti: " << ti << "\n";
 ...
 //         }
          ...
          /*
          std::string path_to_sig = "./signal_1.txt";
              
              double ss[num_clocks][MAXJW];
              
          std::ifstream inFilet("signal_1.txt");
         // inFilet.open (path_to_sig.c_str());
         //     if(inFilet.is_open()){
                  for(int i = 0; i<num_clocks; i++){
                      for(int j = 0; j<JW;j++){

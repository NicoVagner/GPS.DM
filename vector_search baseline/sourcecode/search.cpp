	  ...
	  for(int iw = input.startweek; iw <= input.endweek; iw++){ // loop through weeks
		for(int id = 0; id < 7; id++){ // loop through days in week
            ...
            std::cout << "Here in search 2" << "\n";
				...
				//The below 2 lines are for our prior distributions -- commented out 2/15/2022 for flat prior run - Jiten 
				/*
				NumericCdfInverse v_prior(path_v_cdf);    //	Generate prior distributions (non-flat priors)
				NumericCdfInverse psi_prior(path_psi_cdf);	// (non-flat psi prior)
				*/
				//The below 2 lines are for flat prior distributions -- added on 2/15/2022 for flat prior run - Jiten
				std::string flatPriors="flat";
				NumericCdfInverse v_prior(flatPriors,0,653);	//	Generate flat prior distributions (max speed 653 km/s)
				NumericCdfInverse psi_prior(flatPriors,0,3.14);	//	(generates psi prior, 0 -> pi)

...
*/
{
 ...
 //   std::cout << "Here 2 JPLreadata" << "\n";
  ...
  /* This is commented out, all of the JPL files seem to end in .clk now (as of 1/7/2021) - Jiten
...
//  if(in_jplweek>=1934) return 0; //don't need to fix!
  for(int i=0;i<num_clocks;i++){
    ...
    for(i=0;i<num_clocks;i++){
       // std::cout << "Hello: " << i << "\n";
//        if(clk[i]=="Cs"){
        //if(clk[i]=="Cs" || i==0 || i==24){           // 01/08/21 to take out the 2 bad clocks in gpsday 17150
//        if(clk[i]=="Cs" || svn[i]=="62" || svn[i]=="63" || svn[i]=="66" || svn[i] =="64" || svn[i] == "67" || svn[i] == "68" || svn[i] == "69" || svn[i] == "71" || svn[i] == "73" || svn[i] =="70" || svn[i] == "26"){           // 01/08/21 to take out the 2 bad clocks in gpsday 17150
	if(clk[i]=="Cs" || svn[i]=="62" || svn[i]=="63" || svn[i]=="66" || svn[i] =="64" || svn[i] == "67" || svn[i] == "68" || svn[i] == "69" || svn[i] == "71" || svn[i] == "73" || svn[i] =="70"){           // 01/08/21 to take out the 2 bad clocks in gpsday 17150
//        if(clk[i]=="Cs" || svn[i]!="62" || svn[i]!="63" || svn[i]!="66" || svn[i] !="64" || svn[i] != "67" || svn[i] != "68" || svn[i] != "69" || svn[i] != "71" || svn[i] != "73" || svn[i] !="70"){           // 01/08/21 to take out the 2 bad clocks in gpsday 17150
//	if(clk[i]=="Cs" || svn[i]=="23" || svn[i]=="34" || svn[i]=="41" || svn[i] =="43" || svn[i] == "44" || svn[i] == "45" || svn[i] == "46" || svn[i] == "47" || svn[i] == "48" || svn[i] =="50"|| svn[i] =="51"|| svn[i] =="52"|| svn[i] =="53"|| svn[i] =="54"|| svn[i] =="55"|| svn[i] =="56"|| svn[i] =="57"|| svn[i] =="58"|| svn[i] =="59"|| svn[i] =="60"|| svn[i] =="61"){           // 01/08/21 to take out the 2 bad clocks in gpsday 17150


//        if(clk[i]=="Cs" || svn[i]=="62"){
//        if(clk[i]=="Cs" || svn[i]=="22"){
            tmpsvn[i] = i;
            tmpsvnord[i] = svn[i];
            cs_count++;
 ...
 //   num_clocks = num_satellites - 1; //We don't care about station clocks!
    ...
    //num_clocks = 2;
    //num_satellites = 29;
    //num_clocks = num_clocks - num_satellites -2;        // to check if helps cov mat for gpsday 17150 (2bad SVN 62&63)
    std::cout << "this is number of all clocks after elim: " << num_clocks << "\n";
    
    std::string gvalPath = "./results/gval19622.out";
...
//    for(int i=0;i<num_clocks+1;i++){
    ...
    //for(int i=0;i<num_clocks+num_satellites+2;i++){         // added 01/08/21 to check if helps cov mat for pos def
...
//    for(int i=0;i<num_clocks+1;i++){
    ...
    std::cout << "Hello at end of readbiasdata:  \n";
    

    // this is to save differenced and centered data to a file 2/17/23 (J.S.)---
    // note: commenting out this block of code does not break anything else :)
    std::cout << "Saving pseudo-frequencies to a file" << std::endl;
    std::string biasOutFilePath = "./results/biasData_" + week + day + ".out";
    std::ofstream biasOutFile(biasOutFilePath);

    for(int clockNumber = 0; clockNumber < num_clocks; clockNumber++){
      int numPoints = bias[clockNumber].size();
      for(int clockPos = 0; clockPos < numPoints; clockPos++){
        biasOutFile << bias[clockNumber][clockPos] << " ";
      }
      biasOutFile << std::endl;
    }
    biasOutFile.close();
    //--------------------------------------------------------------------------
...
*/
{
  //std::string ext=".clk_30s"; commented out on 1/7/2021 - Jiten
  std::string ext=".clk";
  std::string zip=".gz";
  //std::string zip=".Z"; commented out on 1/7/2021 - Jiten

  /* All of the JPL files seem to end in ".clk" now (as of 1/7/2022) - Jiten
...
*/
{
  ...
  for(int i=0;i<num_receivers;i++){//only do recievers/stations
    ...
    for(std::size_t l=0; l<station_map.size(); l++){
      ...
      int beg, end;
      std::cout << "sta_clock: " << sta_clock << "\n";		// added on 12/17/21 by tday
      //convert the dates from the map array to jpl wwwwd (int) format:
      beg = MSC_ymdtowd(ini_date);
      if(fin_date=="c"){
        end = 999999; //just really big number. Assignment still current.
...
// added 03/08/21 by tday to change directories of the ecipath after gpsday 18816
  //std::string ecipath = "/ssd/tday/jpl30s/"; //new eci directory (uncomment this after gpsday 18816)
 
  std::string ecipath = jpl_file_dir; //assume same directory (commented out after gpsday 18816)

// end of additions for after gpsday 18816
...
*/
{
  ...
  if(MSC_fexists(filename)){
    ...
    if(1)std::cout<<"~ WARNING: ECI satellite file: ecisat"<<week<<day
                        <<".pos NOT found!\n";
    std::ofstream oFile;
  ...
  //reSizeVec(tempX,num_clocks+num_satellites+2,num_epochs,iXYZ);    // added 01/08/21 for all Cs clock elim plus 2 bad clocks
  ...
  //std::vector<int> testSVN(num_clocks);
    ...
    //std::vector<int> testSVN(num_clocks+num_satellites+2);       // added 01/08/21 for all Cs clock elim plus 2 bad clocks
  ...
  inFile.close();
    ...
    // need some statement that checks the SVN from jpl30s to
    // the EciSat.pos SVN's in order to eliminate that SVN from the network.
    // 11/24/20 tday (maybe helpful)
    ...
    // added on 11/23/20 to check why ecisat is less than the jpl info
    if(eciSat!=num_clocks+num_satellites){
    //if(eciSat!=num_clocks+num_satellites+2){                  // 01/08/21 for 2 bad clock removal
      //if(verbose){
      if(1){
        std::cout<<"Warning 732 in readECI: jpl"<<week+day
                 <<": eciSat=/=total num_satellites !!!"
                 <<" -- ("<<eciSat<<" =/= "<<num_clocks+num_satellites<<")\n";
      }
      std::ofstream oFile;
      std::string filename="ECI-faillog.txt";
      oFile.open (filename.c_str(),std::ios_base::app);
      oFile<<"eciSat=/=num_satellites: "<<week+day<<". eciSat="<<eciSat
           <<", allnum_satellites="<<num_clocks+num_satellites<<"\n";
      oFile.close();
    }
    // end of addition on 11/23/20 tday
    
    int difeci = (num_clocks+num_satellites) - eciSat;
    //int difeci = (num_clocks+num_satellites+2) - eciSat;        // 01/08/21 for 2 bad clock removal
    
    std::sort(tmpsvnord.begin(),tmpsvnord.end());
//    for(int i=0;i<(eciSat - num_satellites);i++){
    for(int i=0;i<(eciSat - num_satellites)+difeci;i++){
        tmpsvnord[i] = "0";
        //std::cout << "new2 tmpsvnord[i]: " << tmpsvnord[i] << "\n";
    }
    
//    for(int i=0;i<eciSat;i++){
    for(int i=0;i<eciSat+difeci;i++){
        std::cout << "i:" << i << " " <<  "new2 tmpsvnord[i]: " << tmpsvnord[i] << "\n";
    }
    
    std::cout <<"after new2" << "\n";
    
    double tmpord[num_satellites];
    //double tmpord[num_satellites+2];            // 01/08/21 for 2 bad clock removal
    int ixt;
    ixt = 0;
    for(int i=0;i<eciSat;i++){
    ...
    //for(int i=0;i<num_satellites+2;i++){        // 01/08/21 for 2 bad clock removal
...
*/
  ...
  //Uses the SVN to match the positions
    std::cout <<" num_recievers: " << num_receivers << "\n";    
    int n;
    n = 0;
  ...
  for(int i=0;i<num_clocks;i++){
    ...
    for(int j=0;j<num_epochs;j++){//Subtract the mean!
      if(ferr[i][j]!=0){
        ...
        std::string gvalPath = "./results2/g10val13100.out";
        std::ofstream gval(gvalPath);
    
        std::string gval2Path = "./results/g10ferr17150.out";
...
// added on 03/19/21 to look at the various network total avg sensitivity tday

    std::string stdPath = "./results2/network_std_gpsday.out";
    std::ofstream nstd(stdPath);



// end of new additions on 03/19/21 tday
  ...
  //  	nstd << sdev[i] << "\n";
    ...
    nstd << avg_sat_sd/good_sat_clocks << "\n";   

    std::cout << "this is avg sdev: " << avg_sat_sd/good_sat_clocks << "\n";
    nstd.close();
...
*/
{
  reSizeVec(acf,num_clocks,acf_points);
    ...
    std::cout << "number of satellites removed in read ACF: " << num_satellites << "\n";
    ...
    std::string acftPath = "./results2/long_2acffilecut17590.out";
    //std::string acftPath = "./results2/acffile16614.out";
    std::ofstream ac(acftPath);
...
//  for (int i=0; i<num_clocks; i++){         // use this one for testMethods.cpp
  for (int i=0; i<num_clocks+num_satellites; i++){    // use this one for search.cpp
...
//    std::cout << "this is SVN in read ACF : " << svn[i] << "\n";
    ...
    if(clk[i]!="Rb" && clk[i]!="H") continue;	// moved this line up for use for gpsdays before 12800 since we need to calc acf
    
    //Calculate the ACF, or read it in from file?
    if(path_to_acf=="calculateACF"){
  //    calculateAcfFromData(i);
    // added on 03/19/21 tday
      if(svn[i]=="22") continue;
      calculateAcfFromData(i,nt);
      nt++;
      if(nt+1>num_clocks){
              break;
      }
    // end of additions on 03/19/21 by tday
      continue;
    }
  ...
  //  if(clk[i]!="Cs" && clk[i]!="Rb" && clk[i]!="H") continue;  // commented out for Cs clock elim ACF from file
   ...
   //   if(clk[i]!="Rb" && clk[i]!="H") continue;		// commented out and moved up so we can use before gpsday 12800
    //if(clk[i]!="Cs" || clk[i]!="Rb" || clk[i]!="H") continue;
      
 //     if(i==0 || i==24) continue;
//      if(svn[i]=="62" || svn[i]=="63" || svn[i]=="66" || svn[i]=="64" || svn[i] == "67" || svn[i] == "68" || svn[i] == "69" || svn[i] == "71" || svn[i] == "73" || svn[i] == "70" || svn[i] == "26") continue;
	if(svn[i]=="62" || svn[i]=="63" || svn[i]=="66" || svn[i]=="64" || svn[i] == "67" || svn[i] == "68" || svn[i] == "69" || svn[i] == "71" || svn[i] == "73" || svn[i] == "70") continue;
//        if(svn[i]=="23" || svn[i]=="34" || svn[i]=="41" || svn[i] =="43" || svn[i] == "44" || svn[i] == "45" || svn[i] == "46" || svn[i] == "47" || svn[i] == "48" || svn[i] =="50"|| svn[i] =="51"|| svn[i] =="52"|| svn[i] =="53"|| svn[i] =="54"|| svn[i] =="55"|| svn[i] =="56"|| svn[i] =="57"|| svn[i] =="58"|| svn[i] =="59"|| svn[i] =="60"|| svn[i] =="61") continue;
//      if(svn[i]=="62") continue;
...
//    std::string acf_fname = prefix+svn[i]+"-"+blk[i]+suffix;  
    std::cout << "clk[i]: " << clk[i] << "	" << "blk[i]: " << blk[i] << "\n";
    std::cout << "acf_fname: " << acf_fname << "\n";
    ...
    for(int i=0;i<num_clocks;i++){
        for(int j=0;j<acf_points;j++){
            std::cout << "this is all acf[i][j]: " << acf[i][j] << "\n";
            ac << acf[i][j] << "\n";
        }
        std::cout <<"\n";
    }
...
//******************************************************************************
int JplGpsData::calculateAcfFromData(int ic,int nt)
/*
171017.
...
03/19/21 added the nt variable so we can calculate the correct clocks after elimination
before gpsday 12800 (otherwise we have acf files for the othe gpsdays
*/
{
...
//  acf[ic][0]=1; //should already be set, but may as well.
  acf[nt][0]=1; //should already be set, but may as well.
  for(int t=1; t<acf_points; t++){
    double temp_acf=0;
    int jmax = num_epochs - t;
    for(int j=jmin; j<jmax; j++){
//      temp_acf += bias[ic][j]*bias[ic][j+t];
      temp_acf += bias[nt][j]*bias[nt][j+t];
    }
//    acf[ic][t] = temp_acf / ( pow(sdev[ic],2)*(num_epochs-t) );
    acf[nt][t] = temp_acf / ( pow(sdev[nt],2)*(num_epochs-t) );
   
    std::cout << "this is acf[nt][t]: " << acf[nt][t] << "\n";
  }
